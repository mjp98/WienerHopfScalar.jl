...
@testset "WienerHopfScalar.jl" begin
    ...
    @testset "nopoles macro" begin
        @test begin
            x = @nopoles TestKernel
            r = TestKernel{Float64}()
            isempty(poles(r))
        end
        @test begin
            x = @noroots TestKernel
            r = TestKernel{Float64}()
            isempty(roots(r))
        end
    ...
    residue(f, x::AbstractArray; kwargs...) = complex.(residue.(f, x; kwargs...))

    function check_root_residue(K::Function, u, n; kwargs...)
        a = root_residue(K, u, n)
        b = residue(z -> inv(evaluate(K, z)), root(K, u, n); kwargs...)
        return a, b
    end

    function check_root(K::Function, u, n)
        z = root(K, u, n)
        return evaluate(K, z + 10 * eps(abs(z)))
    end

    function check_pole(K::Function, u, n)
        z = pole(K, u, n)
        return evaluate(K, z + 10 * eps(abs(z)))
    end

    function check_pole_residue(K::Function, u, n; kwargs...)
        a = pole_residue(K, u, n)
        b = residue(z -> evaluate(K, z), pole(K, u, n); kwargs...)
        return a, b
    end

    import WienerHopfScalar: coshγ_root, overcoshγ_residue
    import WienerHopfScalar: γsinhγ_root, overγsinhγ_residue

    @testset "Duct Kernel" begin
        k = 0.4
        h = 0.2
        ...
        @testset "Roots" begin
            for u in (true,false)
                for n in 0:10
                    ...
                    Rz = residue(z->inv(K(z)),z)
                    @test Rz ≈ overcoshγ_residue(u,n,k,h) rtol = 1e-3
                end
            end

            for u in (true,false)
                for n in 0:10
                    z = γsinhγ_root(u,n,k,h)
                    γ = GammaKernel(k)
                    K = z -> γ(z)*sinh(γ(z)*h)
                    ...
                    Rz = residue(z->inv(K(z)),z)
                    @test Rz ≈ overγsinhγ_residue(u,n,k,h) rtol = 1e-3
                end
            end
        end
